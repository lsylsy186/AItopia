...
const getColumns = (payload: any) => {
  const { edit, remove } = payload;
  return [
    {
      title: '模块 ID',
      dataIndex: 'id',
    ...
    // },
    {
      ...
      render: (_: any, record: any) => {
        ...
        return (
          <span>
            <Typography.Link className="mr-2" onClick={() => edit(record)}>
              编辑
            </Typography.Link>
...
export const Workspace = () => {
  const { callFetchRoleList, roleList, callAddRole, callModRole, callRemoveRole } = useModel('role');
  ...
  const onFinish = async (values: any) => {
    ...
    if (isEdit) {
      ...
      result = await callModRole(payload);
    } else {
      ...
      result = await callAddRole(payload);
    }
    callFetchRoleList();
  ...
  const onDeleteSubmit = async () => {
    const result = await callRemoveRole({ id: deleteId });
    callFetchRoleList();
    onCancelDeleteModal();
  }
  ...
  const edit = (record: any) => {
    ...
    setShowCreateRoleModal(true);
    setEditId(record.id);
    const productLine = record.productLine.map((value: string) => ({ label: value, value }));
    form.setFieldsValue({ ...record, productLine });
  };
  const remove = (record: any) => {
    setShowDeleteRoleModal(true);
    setDeleteId(record.id);
  };
  const columns = getColumns({ edit, remove });
